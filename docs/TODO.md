...
### 3-5. lint 재발 방지 장치를 붙일지 — pre-commit 훅 / CI

2026-09-03에 lint를 0건으로 정리했지만(커밋 `3835e20`) **CI가 없어서 막아주는 게 아무것도 없다.**
직전까지 127~285건이 쌓여 있던 게 정확히 그 이유다. 사용자 판단으로 이번엔 아무것도 안 붙였다.

선택지는 husky + lint-staged(변경 파일만, 로컬에서 즉시) / GitHub Actions(전체, 우회 불가) /
둘 다. 다시 쌓이기 시작하면 그때 붙인다.
