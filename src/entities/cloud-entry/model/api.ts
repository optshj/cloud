...
export async function fetchEntries(): Promise<CloudEntry[]> {
  ...
  const { data: rows, error } = await supabase
    .from("entry_feed")
    .select("id, entry_date, location_dong, tag, comment, photo_path, likes_count")
...
export async function toggleLikeRemote(entryId: string, currentlyLiked: boolean): Promise<void> {
  ...
  if (currentlyLiked) {
    const { error } = await supabase.from("entry_likes").delete().eq("entry_id", entryId).eq("user_id", userId);
    if (error) throw error;
  } else {
...
export async function reportEntryRemote(entryId: string): Promise<void> {
  ...
  const { error } = await supabase.from("entry_reports").insert({ entry_id: entryId, reporter_id: userId });
